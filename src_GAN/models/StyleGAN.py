...
class Generator(nn.Module):
    ...
    def forward(self, z,text_emb=None):
        #Implementing conditioning on text embedding
        z = torch.cat([z, text_emb], dim=1)
        linear = nn.Linear(512, 256)
        linear.to(text_emb.device)
        z = z.to(text_emb.device)
        z=linear(z)
        ...
        w = self.mapping(z)
        # print(f"final forward shape with text emb{w.shape}")
...
class Discriminator(nn.Module):
    def __init__(self,
        ...
        #self.linear_embedding = nn.Linear(512, self.map_channels[last_res] * last_res ** 2)
    ...
    def forward(self, img,text_emb):
        ...
        #print(f" original image shape {img.shape }and text emb shape{text_emb.shape}")
        #embedding shape [1, 256]
        expand_linear = nn.Linear(256, 256*256)
        expand_linear.to(img.device)
        text_emb=text_emb.to(img.device)
        expanded_emb= expand_linear(text_emb).to(img.device)
        #print("Shape after linear expansion:", expanded_emb.shape)
        expanded_emb = expanded_emb.view(1, 1, 256, 256)
        expanded_embedding = expanded_emb.expand(img.size(0), -1, -1, -1)
        img = torch.cat([img, expanded_embedding], dim=1) 
        reduce_channels=nn.Conv2d(4, 3, kernel_size=1)
        reduce_channels.to(img.device)
        img=img.to(img.device)
        img = reduce_channels(img)
